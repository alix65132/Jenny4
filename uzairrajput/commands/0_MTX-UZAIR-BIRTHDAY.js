...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'birthday.jpeg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/wS1sCoM.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/birthday.jpeg");
  ...
  batgiam_img.composite(circleOne.resize(230, 230), 740, 120).composite(circleTwo.resize(303, 303), 149, 95);
