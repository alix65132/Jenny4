...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'frame8.jpeg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/jqyQS5m.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/frame8.jpeg");
  ...
  batgiam_img.composite(circleOne.resize(500, 500), 17000, 540).composite(circleTwo.resize(350, 350), 154, 169);
