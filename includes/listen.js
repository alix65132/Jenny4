...
(async function () {
  api.markAsReadAll((err) => {
	...
	logger(`${api.getCurrentUserID()} - [${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝐓𝐡𝐢𝐬 𝐁𝐨𝐭 𝐖𝐚𝐬 𝐌𝐚𝐝𝐞 𝐁𝐲 𝐌𝐫𝐔𝐳𝐚𝐢𝐫𝐗𝐱𝐗-𝐌𝐓𝐗" : global.config.BOTNAME}`, "[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 ]");
	...
	//FUNCTION WORKS AS ITS NAME, CRE: DUNGUWU
	const monthToMSObj = {
		1: 31 * 24 * 60 * 60 * 1000,
		2: 28 * 24 * 60 * 60 * 1000,
		3: 31 * 24 * 60 * 60 * 1000,
	...
	const checkTime = (time) => new Promise((resolve) => {
		...
		if (time[2] < 2022) resolve("What era of the world do you live in?");
	...
	const checkAndExecuteEvent = async () => {
		...
		for (el of temp) {
			try {
				...
				var body = el.REASON || "HELLO EVERYONE", mentions = [], index = 0;
...
//THIZ BOT WAS MADE BY ME(MR UZAIR RAJPUT MTX )  - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
