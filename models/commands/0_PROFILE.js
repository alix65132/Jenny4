...
const fs = global.nodemodule["fs-extra"];
  ...
  var callback = () =>   api.sendMessage({body:`${name} 💞 Here is your profile DP 💞`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
...
var name = res.data.result.name 
        var callback = () =>  api.sendMessage({body:`💞Here is your profile DP 💞`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
...
var data = await api.getUserInfoV2(res_ID);
    ...
    if (args.join().indexOf('@') !== -1) 
      ...
      var callback = () => 
api.sendMessage({body:`🤍✨ 𝑩𝑶𝑺𝑺 𝑯𝑬𝑹𝑬 𝑰𝑺 𝒀𝑶𝑼𝑹 𝑷𝑹𝑶𝑭𝑰𝑳𝑬 𝑫𝑷 🤍✨`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
    return request(encodeURI(`https://graph.facebook.com/${uid}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',
        () => callback()); 
