...
module.exports.run = async ({ event, api, args }) => {
  ...
  if(args[0] == "clear"){
    ...
    if(!author) return api.sendMessage('No boards have been created in your group yet.', threadID, messageID);
    if (senderID != author) return api.sendMessage('Author only '+author+' chessboard: can only finish this chessboard', threadID, messageID);
    global.game[threadID] = {};
    return api.sendMessage('Checkers table removed!!', threadID, messageID);
