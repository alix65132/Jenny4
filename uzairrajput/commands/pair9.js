...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'uzair/mtx', 'uzairarrb.jpg');
    if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/iubdkZH.jpeg", path);
}
...
async function makeImage({ one, two }) {
    ...
    let pairing_img = await jimp.read(__root + "/uzairarrb.jpg");
    ...
    pairing_img.composite(circleOne.resize(500, 500), 450, 400).composite(circleTwo.resize(484, 484), 2253, 945);
