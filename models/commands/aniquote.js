...
var mention = Object.keys(event.mentions);
 ...
 var uzairrajput3 = ['1', '2'];
 var uzairrajput = uzairrajput3[Math.floor(Math.random() * uzairrajput3.length)];

if (Object.keys(event.mentions).length == 1) {
  if (uzairrajput == 2 ) {
      api.sendMessage(`Send a message to ${event.mentions[mention].replace("@", "")}:\n${uzairrajput2[Math.floor(Math.random() * uzairrajput2.length)]}`,threadID, messageID);
  }
  if (uzairrajput == 1 ) {
      api.sendMessage(`Send a message to ${event.mentions[mention].replace("@", "")}:\n${uzairrajput1[Math.floor(Math.random() * uzairrajput1.length)]}`, threadID, messageID);
  }
}
else {
  if (uzairrajput == 2) {
   api.sendMessage(`${uzairrajput2[Math.floor(Math.random() * uzairrajput2.length)]}`,threadID, messageID); 
  }
  if (uzairrajput == 1 ) {
   api.sendMessage(`${uzairrajput1[Math.floor(Math.random() * uzairrajput1.length)]}`, threadID, messageID);
  }
}
