...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'uzair/mtx', 'arob.png');
    if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/Vti06t4.jpeg", path);
}
...
async function makeImage({ one, two }) {
    ...
    let pairing_img = await jimp.read(__root + "/arob.png");
    ...
    pairing_img.composite(circleOne.resize(208, 208), 142, 141).composite(circleTwo.resize(210, 210), 520, 140);
