...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'frame11.jpg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/3M9u1pR.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/frame11.jpg");
  ...
  batgiam_img.composite(circleOne.resize(500, 500), 17000, 540).composite(circleTwo.resize(940, 940), 1205, 1150);
