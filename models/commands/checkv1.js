...
module.exports.config = {
  ...
  commandCategory: "for user", // Thuộc vào nhóm nào: system, other, game-sp, game-mp, random-img, edit-img, media, economy, ...
...
const tt = (yesterDay, toDay, time) => new Promise((resolve) => {
  if(yesterDay == 0) resolve("no statistics yet");
  if(toDay == 0) resolve("no statistics yet");
...
module.exports.handleEvent = async ({ api, event, args, handleEvent }) => {
  ...
  var timeVN = moment().tz('Asia/Karachi').format('DD_MM_YYYY_HH_mm_ss');
...
module.exports.run = async ({ api, event, args, Currencies, Users }) => {
  ...
  if(args[0] == "all") {
    ...
    msg += `\n» Total number of messages in the box last week: ${sum}`;
    ...
    if(isNaN(test)) {
      msg = "Unclear statistics";
    } else {
      msg = await tt(yesterDay, today, time[3]);
    }
    return api.sendMessage(`=== ${nameUser} ===\n🙆‍♂️ 𝐓𝐨𝐭𝐚𝐥 𝐦𝐞𝐬𝐬𝐚𝐠𝐞𝐬: ${(typeof countMess.exp == "undefined") ? 0 : countMess.exp}\n🐸 𝐼𝑛𝑡𝑒𝑟𝑎𝑐𝑡𝑖𝑜𝑛 𝑙𝑒𝑣𝑒𝑙: 𝑠𝑡𝑎𝑡𝑖𝑠𝑡𝑖𝑐𝑠 𝑎𝑟𝑒 𝑢𝑛𝑐𝑙𝑒𝑎𝑟: ${msg}\n⏱️ 𝑇𝑜𝑡𝑎𝑙 𝑚𝑒𝑠𝑠𝑎𝑔𝑒𝑠 𝑦𝑒𝑠𝑡𝑒𝑟𝑑𝑎𝑦: ${yesterDay}\n⏰ 𝑇𝑜𝑡𝑎𝑙 𝑚𝑒𝑠𝑠𝑎𝑔𝑒𝑠 𝑡𝑜𝑑𝑎𝑦: ${today}\n🖤 𝑇𝑜𝑡𝑎𝑙 𝑚𝑒𝑠𝑠𝑎𝑔𝑒𝑠 𝑡ℎ𝑖𝑠 𝑤𝑒𝑒𝑘: ${weekDay}`, threadID, messageID);
...
module.exports.config = {
  ...
  credits: "uzairrajput", // Công nhận module sở hữu là ai
  description: "JUST CHECKTT", // Thông tin chi tiết về lệnh
  commandCategory: "For users", // Thuộc vào nhóm nào: sy Unclear statisticsstem, other, game-sp, game-mp, random-img, edit-img, media, economy, ...
  usages: "[all]", // Cách sử dụng lệnh
  cooldowns: 5, // Thời gian một người có thể lặp lại lệnh
};
