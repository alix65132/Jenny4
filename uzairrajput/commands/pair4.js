...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'uzair/mtx', 'arb.jpeg');
    if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/648eQc8.jpeg", path);
}
async function makeImage({ one, two }) {
    ...
    let pairing_img = await jimp.read(__root + "/arb.jpeg");
    ...
    pairing_img.composite(circleOne.resize(215, 215), 9, 61).composite(circleTwo.resize(210, 210), 264, 75);
