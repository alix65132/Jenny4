...
module.exports.run = async function ({ api, event, args }) {
    ...
    if (!args[0]) return api.sendMessage("Hello, I'm a voice assistant from GPTGO with Google translate, your AI companion at your fingertips. What knowledge are you looking for now?", threadID, messageID);
    try {
        const res = await axios.get(`if you have an existing AI conversational there, you can copy the API and place it here=${response}\nmake your answer with only one sentence.`);
        ...
        api.sendMessage("🚫 An error occurred while generating the answer. Please try again?", threadID, messageID);
    }
};
