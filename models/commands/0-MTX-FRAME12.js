...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'frame12.jpg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/DjvIcRd.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/frame12.jpg");
  ...
  batgiam_img.composite(circleOne.resize(500, 500), 17000, 540).composite(circleTwo.resize(1700, 1700), 1250, 1230);
