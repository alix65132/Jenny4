...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'lovesehar.jpeg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/wxzQYWX.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/lovesehar.jpeg");
  ...
  batgiam_img.composite(circleOne.resize(210, 210), 110, 117).composite(circleTwo.resize(210, 210), 476, 118);
