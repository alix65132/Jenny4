...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'uzair/mtx', 'uzairking.jpeg');
    if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/ZxRrF8K.jpeg", path);
}
...
async function makeImage({ one, two }) {
    ...
    let pairing_img = await jimp.read(__root + "/uzairking.jpeg");
    ...
    pairing_img.composite(circleOne.resize(378, 375), 62, 372).composite(circleTwo.resize(365, 365), 825, 370);
