...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'frame7.jpg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/ZqAvOzW.jpeg", path);
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/frame7.jpg");
  ...
  batgiam_img.composite(circleOne.resize(388, 388), 168, 445).composite(circleTwo.resize(395, 395), 669, 445);
