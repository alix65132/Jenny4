...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'uzair/mtx', 'arb.png');
    if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/tYztnXw.jpeg", path);
}
...
async function makeImage({ one, two }) {
    ...
    let pairing_img = await jimp.read(__root + "/arb.png");
    ...
    pairing_img.composite(circleOne.resize(258, 258), 116, 110).composite(circleTwo.resize(247, 247), 478, 115);
